...
class EmotionRecognition:
    ...
    def build_network(self):
        ...
        self.model = tflearn.DNN(
            self.network,
            checkpoint_path='model/emotion_recognition',
            max_checkpoints=1,
    ...
    def save_model(self):
        self.model.save(join("model", "emotion_recognition"))
        print('[+] Model trained and saved at ' + "emotion_recognition")

    def load_model(self):
        if isfile(join("model", "emotion_recognition")):
            self.model.load(join("model", "emotion_recognition"))
        print('[+] Model loaded from ' + "emotion_recognition")

    def predict(self, image):
        if image is None:
            return None
        image = image.reshape([-1, self.face_size, self.face_size, 1])
